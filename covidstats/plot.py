...
def generate_week_cases_plot(
    ...
    week_cases_plot = sns.lineplot(x = 'date', y = 'value', hue = 'variable', hue_order = hue_order, palette = palette, data = plot_df)
...
def generate_week_places_cases_plot(df = data.get_week_places_cases_df()):
    ...
    week_places_cases_plot = sns.lineplot(data = df, x = 'date', y = 'infected_avg', hue = 'place', hue_order = draw_order, palette = draw_colors_order)
...
def generate_14_days_prediction_plot(
        ...
        rt_df,
        predicted_rt_df):
    cases_plot = sns.lineplot(data = cases_df, x = cases_df.date.append(predicted_cases_df.index.to_series(), ignore_index = True), y = 'infected', color = 'orange', label = 'Отчетени случаи')
    predicted_cases_r_increase_plot = sns.lineplot(data = predicted_cases_df, x = predicted_cases_df.index, y = 'increase_cases', color = 'red', label = 'Предсказани случаи (Rt* + 5%)')
    predicted_cases_r_retention_plot = sns.lineplot(data = predicted_cases_df, x = predicted_cases_df.index, y = 'predicted_cases', color = 'purple', label = 'Предсказани случаи (Rt*)')
    predicted_cases_r_decline_plot = sns.lineplot(data = predicted_cases_df, x = predicted_cases_df.index, y = 'decline_cases', color = 'olive', label = 'Предсказани случаи (Rt* - 5%)')
    ...
    with sns.axes_style('ticks'):
        ...
        rt_plot = sns.lineplot(data = rt_df, x = rt_df.index, y = 'Q0.5', ax = common_ax, color = 'lightblue', label = 'Моментно репродуктивно число (Rt)')
        ...
        rt_prediction_retention_plot = sns.lineplot(data = predicted_rt_df, x = predicted_rt_df.index, y = 'predicted_rt', ax = common_ax, color = 'darkblue', label = 'Предсказано моментно репродуктивно число (Rt*)')
        ...
        plt.legend(lines + lines2, labels + labels2, loc = 'lower center', bbox_to_anchor=(0.5, -0.35), ncol = 2, frameon = False)

    return cases_plot

def generate_weekly_14_days_prediction_plot_for_date(start_date, week_cases_df = data.get_week_cases_df(), date_diff_cases_df = data.get_date_diff_cases_df()):
    ...
    predicted_cases_df = data.build_predicted_cases_df(rt_df[rt_df.index < start_datetime]['cases'], predicted_rts_df, start_datetime)
