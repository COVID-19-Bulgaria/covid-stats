...
def get_week_cases_df():
    week_cases_df = pd.read_csv('https://raw.githubusercontent.com/COVID-19-Bulgaria/covid-database/master/Bulgaria/WeekCasesDataset.csv')
    week_cases_df['date'] = week_cases_df.apply(lambda row: pd.to_datetime(dt.date.fromisocalendar(row.year, row.week, 7)), axis = 1)
    
    return week_cases_df

def get_week_places_cases_df():
    week_places_cases_df = pd.read_csv('https://raw.githubusercontent.com/COVID-19-Bulgaria/covid-database/master/Bulgaria/WeekPlacesCasesDataset.csv')
    week_places_cases_df['date'] = week_places_cases_df.apply(lambda row: dt.date.fromisocalendar(row.year, row.week, 7), axis = 1)

    return week_places_cases_df

def get_active_cases_df():
    active_cases_dataset = pd.read_json('https://raw.githubusercontent.com/COVID-19-Bulgaria/covid-database/master/Bulgaria/DateActiveCasesDataset.json')
...
def get_date_diff_cases_df():
    date_cases_dataset = pd.read_json('https://raw.githubusercontent.com/COVID-19-Bulgaria/covid-database/master/Bulgaria/DateDiffCasesDataset.json')
    
    for column in ['infected', 'cured', 'fatal', 'hospitalized', 'intensive_care', 'medical_staff', 'pcr_tests', 'antigen_tests', 'vaccinated']:
...
def build_weekly_predicted_cases_df(week_cases_df, predicted_cases_df, start_date):
    ...
    if end_of_week_date >= start_date:
        if end_of_week_date in week_cases_df.index:
